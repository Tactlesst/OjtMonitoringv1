...
export default function StudentSidebar({ selectedPage, onPageChange, user }) {
  ...
  useEffect(() => {
    if (user?.userId) {
      const fetchStudentData = async () => {
        try {
          const response = await axios.get(`/api/student/${user.userId}`);
          setStudentData(response.data); // Store fetched student data
        } catch (error) {
      ...
      <div>
        ...
        <nav className="p-4 space-y-2">
          <button
            ...
            className={`w-full text-left px-4 py-2 rounded-md hover:bg-blue-600 ${selectedPage === 'home' ? 'bg-blue-600' : ''} flex items-center`}
          >
          ...
          <button
            ...
            className={`w-full text-left px-4 py-2 rounded-md hover:bg-blue-600 ${selectedPage === 'attendance' ? 'bg-blue-600' : ''} flex items-center`}
          >
          ...
          <button
            ...
            className={`w-full text-left px-4 py-2 rounded-md hover:bg-blue-600 ${selectedPage === 'guidelines' ? 'bg-blue-600' : ''} flex items-center`}
          >
          ...
          <button
            ...
            className={`w-full text-left px-4 py-2 rounded-md hover:bg-blue-600 ${selectedPage === 'logs' ? 'bg-blue-600' : ''} flex items-center`}
          >
